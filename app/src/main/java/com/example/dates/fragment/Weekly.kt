...
class Weekly : Fragment() {
    ...
    override fun onCreateView(
        ...
        if (isConnected.isInternetAvailable()) {
            viewModel.getWeeklyDate()
        } else {
            binding.prog.visibility = View.GONE
            binding.weeklyDatesRv.visibility = View.GONE
        ...
        viewModel.datesResponse.observe(requireContext() as LifecycleOwner) { response ->
            if (response.isSuccessful) {
                ...
                if (res.date.isNotEmpty()) {
                    binding.prog.visibility = View.GONE
                    binding.weeklyDatesRv.visibility = View.VISIBLE

                    list = res.date
                    adapter = ManagerDatesAdapter(requireContext(), requireActivity(), list)
                    binding.weeklyDatesRv.layoutManager = LinearLayoutManager(requireContext())
                    binding.weeklyDatesRv.adapter = adapter
                } else {
                    binding.prog.visibility = View.GONE
                    binding.weeklyDatesRv.visibility = View.GONE
